...
class FARPhotoViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate,UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        databaseRef.child("FAR Images").queryOrderedByKey().observe(.childAdded, with: {
            snapshot in
            
            let name = snapshot.value as! String
            let imageRef = storageRef.child("FAR Images").child(name)
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "photoCell", for: indexPath) as! FARPhotoTableViewCell
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        let storageRef = FIRStorage.storage().reference().child("FAR Images")
        ...
        let databaseRef = FIRDatabase.database().reference()
        databaseRef.child("FAR Images").childByAutoId().setValue("\(imageName).png")
    }
