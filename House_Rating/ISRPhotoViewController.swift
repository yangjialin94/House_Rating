//
//  ISRPhotoViewController.swift
//  House_Rating
//
//  Created by Jialin Yang on 11/27/16.
...
class ISRPhotoViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate,UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        databaseRef.child("ISR Images").queryOrderedByKey().observe(.childAdded, with: {
            snapshot in
            
            let name = snapshot.value as! String
            let imageRef = storageRef.child("ISR Images").child(name)
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "photoCell", for: indexPath) as! ISRPhotoTableViewCell
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        let storageRef = FIRStorage.storage().reference().child("ISR Images")
        ...
        let databaseRef = FIRDatabase.database().reference()
        databaseRef.child("ISR Images").childByAutoId().setValue("\(imageName).png")
    }
