//
//  ISRViewController.swift
//  House_Rating
//
//  Created by Jialin Yang on 11/2/16.
...
class ISRViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        databaseRef.child("ISR").queryOrderedByKey().observe(.childAdded, with: { snapshot in
            ...
            self.dinningScore.text = "Dinning Hall: " + String(format: "%.1f", dScore) + " / 5.0"
            self.furnitureScore.text = "Furniture: " + String(format: "%.1f", fScore) + " / 5.0"
            self.conveniencyScore.text = "Conveniency: " + String(format: "%.1f", cScore) + " / 5.0"
        })
    }
