...
class LARPhotoViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate,UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        databaseRef.child("LAR Images").queryOrderedByKey().observe(.childAdded, with: {
            snapshot in
            
            let name = snapshot.value as! String
            let imageRef = storageRef.child("LAR Images").child(name)
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "photoCell", for: indexPath) as! LARPhotoTableViewCell
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        let storageRef = FIRStorage.storage().reference().child("LAR Images")
        ...
        let databaseRef = FIRDatabase.database().reference()
        databaseRef.child("LAR Images").childByAutoId().setValue("\(imageName).png")
    }
