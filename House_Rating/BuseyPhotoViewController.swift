//
//  BuseyPhotoViewController.swift
//  House_Rating
//
...
class BuseyPhotoViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate,UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        databaseRef.child("Busey-Evans Images").queryOrderedByKey().observe(.childAdded, with: {
            snapshot in
            
            let name = snapshot.value as! String
            let imageRef = storageRef.child("Busey-Evans Images").child(name)
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "photoCell", for: indexPath) as! BuseyPhotoTableViewCell
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        let storageRef = FIRStorage.storage().reference().child("Busey-Evans Images")
        ...
        let databaseRef = FIRDatabase.database().reference()
        databaseRef.child("Busey-Evans Images").childByAutoId().setValue("\(imageName).png")
    }
